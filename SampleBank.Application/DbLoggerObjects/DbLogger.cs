...
namespace SampleBank.Application.DbLoggerObjects
{
    /// <summary>
    /// Class for Logger
    /// </summary>
        ...
        public void Log<TState>(LogLevel logLevel, EventId eventId, TState state, Exception exception, Func<TState, Exception, string> formatter)
        {
            ...
            using (var connection = new SqlConnection(_dbLoggerProvider.Options.ConnectionString))
            {
                ...
                using (var command = new SqlCommand())
                {
                    ...
                    command.CommandText = string.Format("INSERT INTO {0} ([Id], [ErrorDetail], [Created]) " +
                        "VALUES (@Id, @ErrorDetail, @Created)",
                        _dbLoggerProvider.Options.LogTable);

                    command.Parameters.Add(new SqlParameter("@Id", Guid.NewGuid()));
                    command.Parameters.Add(new SqlParameter("@ErrorDetail",
